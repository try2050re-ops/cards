...
interface Lines20TableProps {
  onAddLine: () => void;
  onEditLine: (line: Line20) => void;
}

export const Lines20Table = ({ onAddLine, onEditLine }: Lines20TableProps) => {
  const [lines, setLines] = useState<Line20[]>([]);
  ...
  const fetchLines = async () => {
    try {
      const { data, error } = await supabase
        .from('lines_20_with_calculations')
        .select('*')
      ...
      console.error('Error fetching lines 20:', error);
      toast({
        title: "خطأ",
        description: "فشل في تحميل بيانات خطوط 20 ميجا",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const getRenewalAlert = (line: Line20) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">خطوط 20 ميجا</h2>
          ...
          <TableBody>
            {lines.map((line, index) => (
              ...
              >
                ...
                <TableCell>
                  <Badge variant="outline" className="bg-blue-50">{line.line_type} ميجا</Badge>
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-2">
      ...
      {lines.length === 0 && (
        <div className="text-center py-12 animate-fade-in">
          <div className="text-muted-foreground text-lg">لا توجد خطوط 20 ميجا</div>
